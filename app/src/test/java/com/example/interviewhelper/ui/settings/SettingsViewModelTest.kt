...
class SettingsViewModelTest {
    ...
    @Test
    fun `立即备份成功提示`() = runTest(mainDispatcher.testDispatcher.scheduler) {
        coEvery { settingsRepository.getWebDavConfig() } returns
            WebDavConfig(serverUrl = "https://example.com")
    ...
    @Test
    fun `从 WebDAV 恢复下载最新备份并覆盖导入`() = runTest(mainDispatcher.testDispatcher.scheduler) {
        coEvery { settingsRepository.getWebDavConfig() } returns
            WebDavConfig(serverUrl = "https://example.com")
        coEvery { webDavDataSource.listFiles(any()) } returns Result.success(
            listOf(
                com.example.interviewhelper.data.model.WebDavFile(
