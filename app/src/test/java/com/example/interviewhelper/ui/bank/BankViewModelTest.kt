...
class BankViewModelTest {
    ...
    fun `分类筛选触发分页查询`() = runTest(mainDispatcher.testDispatcher.scheduler) {
        ...
        viewModel.onCategoryClick("Agent 智能体")
        advanceUntilIdle()
    ...
    fun `难度筛选触发分页查询`() = runTest(mainDispatcher.testDispatcher.scheduler) {
        ...
        // 新语义：难度内嵌于分类按钮，先切换分类，再展开该分类的难度选择，最后选难度
        viewModel.onCategoryClick("Agent 智能体")
        viewModel.onCategoryClick("Agent 智能体") // 再次点击当前分类展开难度选择
        viewModel.setCategoryDifficulty("高级")
        advanceUntilIdle()

        collectJob.cancel()
        // 难度按分类独立记忆，且分页查询携带分类参数
        assertEquals("高级", viewModel.uiState.value.perCategoryDifficulty["Agent 智能体"])
        verify { questionRepository.getPagedQuestions("Agent 智能体", "高级", null, false) }
    }
