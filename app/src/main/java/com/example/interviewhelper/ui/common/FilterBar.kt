 ...
 * - 仅搜索 + 可见开关（无分类/难度下拉）时，两者同处一行，搜索框占满剩余宽度
 *
...
fun FilterBar(
    ...
    Surface(
        ...
        Column(
            ...
            // 仅搜索 + 开关（无分类/难度下拉）时，搜索与开关同行显示
            val inlineSearchRow = showSearch && selectedCategory == null && selectedDifficulty == null

            // 搜索框独占一行（存在分类/难度下拉时的布局）
                ...
                Row(
                    ...
                    if (showVisibleSwitch) {
                        VisibleSwitchRow(
                            ...
                            onVisibleOnlyChange = { onVisibleOnlyChange?.invoke(it) }
                        )
                    }
                }
            } else {
...
/** "仅看可见"开关（保留 visibleOnlySwitch testTag） */
@Composable
private fun VisibleSwitchRow(
    visibleOnly: Boolean,
    onVisibleOnlyChange: (Boolean) -> Unit,
    modifier: Modifier = Modifier
) {
    Row(
        verticalAlignment = Alignment.CenterVertically,
        ...
        Switch(
            checked = visibleOnly,
            onCheckedChange = onVisibleOnlyChange,
